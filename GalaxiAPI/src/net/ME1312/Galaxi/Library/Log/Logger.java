...
public final class Logger {
    ...
    static void log(LogStream stream, String original) {
        if (service != null) service.submit(() -> {
            try {
                ...
                for (String message : messages) {
                    ...
                    if (response == null || response == Boolean.TRUE) {
                        if (terminated || last != stream) {
                            ...
                            if (color && stream.getLevel().getColor() != null) result.append(stream.getLevel().getColor().toString());
                            result.append(prefix);
                            if (color && stream.getLevel().getColor() != null) result.append(Ansi.ansi().reset().toString());
                        }
                        last = stream;
                        logged = true;
                        terminated = false;
                ...
                if (result.length() > 0) stream.stream.value().write(result.toString());
            } catch (Throwable e) {
